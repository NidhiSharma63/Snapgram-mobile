...
const SinglePostComponent = ({id}: {id: string}) => {
  ...
  const renderPost = ({item}: {item: any}) => {
    const user = usersData?.find(u => u?._id === item?.userId);
    return (
      <Pressable onPress={() => handleNavigateToSinglePost(item._id)}>
        <View style={style.explorePost}>
          ...
          <View style={style.userData}>
            ...
            <Text style={style.textPrimary}>{user?.username || 'Unknown'}</Text>
          </View>
        </View>
      </Pressable>
    );
  ...
  // 👇 Main post ko header me daalenge
  const renderHeader = () => (
    <View style={style.postCard}>
      ...
      {/* image */}
      <Image style={style.postImg} source={{uri: data?.file}} />
